...
for ( s in 1:nspp) { 
	...
	for (rs in 1:(nspp)){
		...
		cc.p=Re(fft((fft(nr2_eq_all)*fkc),inverse=T)/((np.xx+0.2*np.xx)*(np.yy+0.2*np.yy)))
		cc.p=rbind(cc.p[ceiling(np.xx/2):(np.xx), ],cc.p[1:floor(np.xx/2), ])
		cc.p=cbind(cc.p[ ,ceiling(np.yy/2):(np.yy) ],cc.p[ ,1:floor(np.yy/2)])
		...
		var_mu_Us.p[s,rs] = var( c((1+cc.p)/mean(1+cc.p,na.rm=T)-1),na.rm=T )
		cov_e_mu_Us.p[s,rs] = cov( c(Frs[,,1]/mean(Frs[,,1],na.rm=T)-1), c( (1+cc.p)/mean((1+cc.p),na.rm=T)-1),use="complete.obs" )
		...
		tryCatch( { cov.tmp = get.fd.cov2D(Frs[,,1], 1+cc.p, l1.p[s,rs], D.p[s,rs], gr1.n.p[s,rs], sr[s], a_rr[s])
